...
let [user,setUser]=useState({"cname":"","emailid":"","gender":"","phonenumber":"","address":"",
...
const signIn = async (event)=> {
  ...
  if(login.typeofuser=="admin"){
    let  adminLoginData = await dispatch(loginSignIn());
          
        if(adminLoginData.payload.emailid==login.emailid && adminLoginData.payload.password==login.password){
  ...
  if(customerLoginResult!=undefined){
    ...
    setError({
      title: 'An error occurred!',
      message: 'Plz select type of user'
    })
    return
...
const signUp= async (event)=> {
  ...
  if(user.cname.length==0){
    ...
    setUser(user => 
      {return {...user,"cid":initialCustomerNumber,"accno":initialAccountNumber,"amount":initialAmount}});
    setStoreData(true);
  }
        ...
        <div className="col-lg-6 mb-5" data-aos="fade-up" data-aos-delay="100">
          <form style={{"marginTop": "0px"}} onSubmit={signUp}>
            ...
            <div className="form-floating mb-2">
              ...
              value={user.cname} 
              name="cname" onChange={(event)=> {
                setUser(user=> {return {...user,"cname":event.target.value}})
              }}/>
            ...
            <div className="form-check mb-3">
              ...
              onChange={(event)=> {
                setUser(user=> {
                    if(event.target.checked){
                      return {...user,"gender":event.target.value};
                    }
                  }
                )
              }
            ...
            <div className="form-floating mb-2">
              ...
              onChange={(event)=> {
                setUser(user=> {return {...user,"phonenumber":event.target.value}})
              }}
              />
